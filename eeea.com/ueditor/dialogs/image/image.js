...
(function () {
    ...
    function OnlineImage(target) {
        this.container = utils.isString(target) ? document.getElementById(target) : target;
        this.selectdiv = utils.isString('selectdiv') ? document.getElementById('selectdiv') : 'selectdiv';
        this.init();
    }
    OnlineImage.prototype = {
        ...
        initContainer: function () {
            ...
            this.select.id = 'selectpath';
            this.option = document.createElement('option');
            domUtils.addClass(this.select, 'select');
            this.option.text = '请选择目录';
            this.option.value = '0';
            this.select.options.add(this.option);
            this.selectdiv.appendChild(this.select);

            this.oInput=document.createElement("input");
            this.oInput.type="hidden";
            this.oInput.id="selectpath_id";
            this.oInput.value="0";
            this.selectdiv.appendChild(this.oInput);
        ...
        initEvents: function () {
            ...
            /* 选中目录 */
            domUtils.on($G('selectdiv'), 'click', function (e) {
                var new_path = $("#selectpath").val();
                var old_path_id = $("#selectpath_id").val();
                if (new_path != old_path_id){
                    $("#selectpath_id").val(new_path);
                    _this.getImageData();
                }
            });
            /*/!* 滚动拉取图片 *!/
        ...
        getpathData: function () {
            var _this = this;
            var url =editor.getActionUrl('listpath'),
                isJsonp = utils.isCrossDomainUrl(url);
            ajax.request(url, {
                'timeout': 100000,
                'dataType': isJsonp ? 'jsonp':'',
