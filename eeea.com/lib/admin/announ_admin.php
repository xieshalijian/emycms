...
if (!defined('ROOT'))
    exit('Can\'t Access !');
class announ_admin extends admin
{
    function init()
    {
        $this->check_pw();
    ...
    function show_action()
    {
        ...
        if (front::get('id')){
            ...
            $this->view->announ = $announcement->getrow($id);
            $tempname = "announ/show.html";


            $tempname=str_replace(".html","",$tempname);
            $url=config::get('site_url')."index.php?case=announ&act=show&pageset=1&isvisual=1&id=".$id.'&url='.lang::getisadmin();;
    ...
    function list_action()
    {
        ...
        if (front::get('catid')){
            ...
            if (!$tempname){
                    $tempname = category::gettemplate($categorydata['catid'],'listtemplate',true,$this->view->_style);
            }
            $tempname=str_replace(".html","",$tempname);
