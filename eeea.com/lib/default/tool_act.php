...
class tool_act extends act
{
    ...
    public function smscodetel_action()
    {
        $mobile = front::$post['tel'];
        $username = front::$post['username'];
        $user_obj = new user();
        $user = $user_obj->getrow(array('username' => $username));
        if($mobile !=  $user['tel']){
            exit(lang('tel_and_user_does_not_match'));
        }
    ...
    function uploadfile_action()
    {
        ...
        if (is_array($_FILES)) {
            ...
            foreach ($_FILES as $name => $file) {
                ...
                if (!$file['name'] || !preg_match('/\.(' . $_file_type . ')$/', $file['name']))
                    continue;
                $isfile_name=config::get('is_attachment_intro');
                $uploads[$name] = $upload->run($file,$isfile_name);
                if (!$uploads[$name]) {
                ...
                //图片上传  插件
                apps::updateimg($uploads[$name],ROOT.$uploads[$name]);
                ...
                if (!$isfile_name && isset($uploads[$name]) && $uploads[$name]) {
                    $rname = basename($uploads[$name]);
                }else{
    ...
    function uploadfile_unionpay_action()
    {
        $res = array();
        $uploads = array();
        if (is_array($_FILES)) {
            $upload = new upload();
            $upload->dir = 'unionpay';
            $upload->url_pre =  'common/plugins/unionpay';
            $upload->new_name =  'acp_sign_inst';
            $upload->max_size = config::get('upload_max_filesize') * 1024 * 1024;
             $_file_type = str_replace(',', '|', config::get('upload_filetype'));
            $upload->type = explode('|', $_file_type);
            foreach ($_FILES as $name => $file) {
                ...
                $rname = preg_replace('%(.*)[\\\\\/](.*)_\d+(\.[a-z]+)$%i', '$2$3', $uploads[$name]);
                $res[$name]['code'] = " 
                document.form1.acpsdk_signCert_path.value=data[key].name; 
                get('file_info').innerHTML=lang('attachment_has_been_saved_size')+data[key].size+'K ';
                        ";
            }
        }
    ...
    //手动升级
    function uploadupdatefile_action()
    {
        ...
        if (is_array($_FILES)) {
            ...
            foreach ($_FILES as $name => $file) {
                ...
                $rname = preg_replace('%(.*)[\\\\\/](.*)_\d+(\.[a-z]+)$%i', '$2$3', $uploads[$name]);
                $res[$name]['code'] = "
                get('attachment_id').value='".$res[$name]['id']."';
                if(!get('attachment_intro').value) {
                get('attachment_intro').value='$rname';
                }
                get('attachment_path').value='".$res[$name]['name']."';
                get('attachment_path_i').innerHTML='".$res[$name]['name']."';
                get('file_info').innerHTML='".lang('attachment_has_been_saved_size')."'+'".$res[$name]['size']."'+'K ';
                        ";
                session::set('attachment_id', $res[$name]['id']);
            }
        }
