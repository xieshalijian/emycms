...
class _PHPRPC_Client {
    ...
    function _connect() {
        if (is_null($this->_proxy)) {
            ...
            $this->_socket = stream_socket_client($host.":".$this->_server['port'], $errno, $errstr, $this->_timeout);
        }
        else {
            $host = (($this->_server['scheme'] == "https") ? "ssl://" : "") . $this->_proxy['host'];
            $this->_socket = stream_socket_client($host.':'.$this->_proxy['port'], $errno, $errstr, $this->_timeout);
        }
