...
{
   if(config::get('guestbook_enable')==1 && (config::get('guestbook_switch')==0 || (session::get('username')!='' && config::get('guestbook_switch')==1))) {
...
function getDescription($archive, $category, $catid, $type,$special=null,$tags=null)
{
    if (isset($archive['description']) && $archive['description']) {
        ...
        if (isset($type['description']) && $type['description']) {
            ...
            return;
        }else if (isset($special['description']) && $special['description']) {
...
{
    if (isset($category[$catid]['description']) && $category[$catid]['description']) {
        return $category[$catid]['description'];
    } else if (isset($category[$catid]['parentid']) && $category[$catid]['parentid']) {
...
{
    if (isset($category[$catid]['keyword']) && $category[$catid]['keyword']) {
...
function getKeywords($archive, $category, $catid, $type,$special=null,$tags=null)
{
    //var_dump($category);
    if (isset($archive['keyword']) && $archive['keyword']) {
        echo $archive['keyword'];
        return;
    } else {
        if (isset($type['keyword']) && $type['keyword']) {
            echo $type['keyword'];
            return;
        }else  if (isset($special['keyword']) && $special['keyword']) {
            echo $special['keyword'];
            return;
        } else  if (isset($tags['keyword']) && $tags['keyword']) {
            echo $tags['keyword'];
            return;
...
function getTitle($archive, $category, $catid, $type,$special=null,$tags=null,$announ=null,$proxy=null)
{

    if(front::$case == 'guestbook'){
        echo config::get("guestbook_title");
        return;
    }
   /* if(front::$case == 'tag' && front::$act == 'show'){
        echo front::get('tag');
        return;
    }*/

    if(front::$case == 'archive' && front::$act == 'search'){
        echo front::post('keyword');
        return;
    }
    if(front::$case == 'comment' && front::$act == 'list'){
        echo config::get("comment_title");
        return;
    }
    ...
    if (!empty($archive['mtitle'])) {
        echo $archive['mtitle'];
        return;
    } elseif (isset($category[$catid]['meta_title']) && $category[$catid]['meta_title'] && isset($archive['title']) && !$archive['title']) {
        echo $category[$catid]['meta_title'];
        return;
    } else {
        if (isset($archive['title']) && !empty($archive['title'])) {
            echo $archive['title'];
            return;
        }

        if (isset($type['meta_title']) && $type['meta_title']) {
            echo $type['meta_title'];
            return;
        } elseif (isset($type['typeid']) && typename($type['typeid'])) {
            echo typename($type['typeid']);
            return;
        }

        if (isset($category[$catid]['meta_title']) && $category[$catid]['meta_title']) {
            echo $category[$catid]['meta_title'];
            return;
        }elseif (isset($special['meta_title']) &&  $special['meta_title']){
            echo $special['meta_title'];
            return;
        }elseif (isset($special['title']) &&  $special['title']){
            echo $special['title'];
            return;
        } elseif (!empty($catid)) {
            echo catname($catid,$proxy);
            return;
        }elseif (isset($tags['meta_title']) &&  $tags['meta_title']){
            echo $tags['meta_title'];
            return;
        } elseif (isset($announ['title']) && $announ['title']){
            echo $announ['title'];
            return;
        }
        echo config::get('fullname');
        return;
    }
}
...
function getform($name)
{
    ...
    if (empty($field)) {
        return lang_admin('form_was_not_found');
    }
    ...
    return template_user(@setting::$var[$name]['myform']['template']);
}
...
{
    if (is_array(front::$user)) {
            $roles = usergroup::getInstance()->getrow(array('groupid' => front::$user['groupid']));
    }else
        $roles = usergroup::getInstance()->getrow(array('groupid' => 1000));

    if ($roles['discount'] != 0) {
        $newprice = (double)$price * (double)$roles['discount'] * 0.1;
    } else {
        $newprice = $price;
    }
...
{
    $tables = array(lang_admin('inherit'), lang_admin('unbound'));
    $tdatabase = new tdatabase();
    $forms = $tdatabase->getTables();
    $showname='cname_'.lang::getisadmin();
    foreach ($forms as $form) {
        if (preg_match('/^' . config::getdatabase('database', 'prefix') . '(my_\w+)/xi', $form['name'], $res))
            $tables[$res[1]] = @setting::$var[$res[1]]['myform'][$showname];
    }
    return $tables;
}
...
function getHtmlRule($cate)
{
    ...
    $htmlrulearr[''] = lang_admin('please_choose');
    foreach ($arr as $v) {
        if ($cate == $v['cate'])
...
function sendMsg($mobile, $content)
{
    ...
    $tmp = explode('@', $tc);
    if ($tmp[0] >= config::getadmin('sms_maxnum') && $tmp[1] == date('Y-m-d')) {
        front::flash(lang_admin('send_failed_check_user_name_password_or_number_of_remaining_entries'));
        return -200;
    }
    ...
    //var_dump($mobile);
    $rs = $client->sendMsg($mobile, $content, config::getadmin('sms_username'), md5(config::getadmin('sms_password')));
    //var_dump($rs);
    if ($rs == '0') {
        $num = $tmp[0] + 1;
...
function put_into_db($tbname, $data)
{
    ...
    $nerrCode='';
    if (!$mysql->query($sql)) {
        $nerrCode .= lang_custom_admin('implementation')."： <font color='blue'>$mysql</font> ".lang_custom_admin('error')."</font><br>";
    }
    return $nerrCode;
    // @mysql_query($sql);
...
{//依次传入，总数，每页显示条目，显示链接总数,当前所在页
    ...
    foreach ($link as $v) {
        if ($v == $current) {
            $link_str .= '<a href="javascript:picload(' . front::get('amid') . ',' . $v . ');"  class="active">' . $v . '</a>';
        } else {
...
{//依次传入，总数，每页显示条目，显示链接总数,当前所在页
    ...
    foreach ($link as $v) {
        if ($v == $current) {
            $link_str .= '<a  href="#" onclick="gotourl(this)"   data-dataurl="' . $url . '&page=' . $v . '" style="color:red">' . $v . '</a>';
        } else {
            $link_str .= '<a href="#" onclick="gotourl(this)"   data-dataurl="' . $url . '&page=' . $v . '">' . $v . '</a>';
        }
    }
    if ($total_page > 10) {
        if (!in_array(1, $link))
            $link_str = '<a href="#" onclick="gotourl(this)"   data-dataurl="' . $url . '&page=1">1...</a>' . $link_str;
        if (!in_array($total_page, $link))
            $link_str .= '<a href="#" onclick="gotourl(this)"   data-dataurl="' . $url . '&page=' . $total_page . '">...' . $total_page . '</a>';
    }
    return $link_str;
}
