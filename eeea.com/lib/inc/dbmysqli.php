...
class dbmysqli
{
    ...
    public static function getInstance($host, $user, $pass, $dbname)
    {
        ...
        if (!self::$me) {
            ...
            self::$me->islink = self::$me->connect("p:".$host, $user, $pass, $dbname);
        }
        return self::$me;
    }
     ...
     */
    public function connect($host, $user, $pass, $dbname, $con_db_port = '3306', $db_charset = 'utf8', $pconnect = 0)
    {
        $this->mysqli = @new mysqli($host, $user, $pass, $dbname,$con_db_port);
        if ($this->mysqli->connect_error) {
            self::halt($this->mysqli->connect_error);
        }
        #if (self::version() > '4.1')
    ...
    {
        return 'mysql'.@$this->mysqli->server_info;
    }
    ...
    function query($sql)
    {
        ...
        if (!$res) {
            $this->halt(lang_admin('query').lang_admin('failure').":\n$sql");
        }
        return $res;
    }
        ...
        if (isset($condition) && is_array($condition)) {
            ...
            foreach ($condition as $key => $value) {
                ...
                if (inject_check($value)) {
                    //echo $condition;
                    event::log('inject',$value);
                    exit('sql inject');
                }
                if (inject_check($key)) {
                    event::log('inject',$key);
                    exit('sql inject');
                }
            ...
            //echo $condition." __ ";
            if (inject_check($condition)) {
                event::log('inject',$condition);
                //echo $condition;
                exit('sql inject');
            }
        }
    ...
    function getFields()
    {
        ...
        //var_dump($fields);
        $this->primary_key = isset($primary_key)?$primary_key:"";
