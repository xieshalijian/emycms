...
class templatetagwap {
    ...
    public function getfilename() {
        return TEMPLATE.'/'.config::get('template_mobile_dir').'/data/templatetag_'.lang::getisadmin().'.php';
    }
    public function savesetting() {
        if (empty(self::$setting))
            return;
        $setting_file=self::getfilename();
        file_put_contents(iconv("utf-8", "gbk",$setting_file),'<?php return '.var_export(self::$setting,true).';');
    }
    ...
    static function tag($tagid) {
        ...
        if (is_array($tag)) {
            ...
            $path=ROOT . '/cache/template/' .config::get('template_mobile_dir'). '/oldmodules'.'/'.$tag['tagfrom'];
    ...
    public function getlisttagcontent($tag,$tag_config) {
        ...
        if (!$tag_config['titlenum']){
            $tag_config['titlenum']=0;
        }
        if (!$tag_config['textnum']){
