...
class attachment_act extends act
{
    ...
    function attachment_js_action()
    {
        ...
        //var_dump($arcpv);
        if (!$arcpv || !$catpv) {
            $link = "<a id='att' title='$name' href='javascript:alert(\"" . lang('without_authorization_can_ not_download') . "\");' class='btn btn-default visual-content-down-link-btn'>" . lang('nowdownload') . "</a>";
            echo tool::text_javascript($link);
            exit;
        } else {
            if (config::get('verifycode')) {
                $link = "<a target='_blank' title='$name' id='att' href='" . url::create('attachment/downfile/aid/' . $aid . '/v/ce') . "' class='btn btn-default visual-content-down-link-btn'>" . lang('nowdownload') . "</a>";
                echo tool::text_javascript($link);
                exit;
            } else {
                $link = "<a target='_blank' title='$name' id='att' href='" . url::create('attachment/down/aid/' . $aid) . "' class='btn btn-default visual-content-down-link-btn'>" . lang('nowdownload') . "</a>";
                echo tool::text_javascript($link);
                exit;
            }
        }
