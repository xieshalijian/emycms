...
<script type="text/javascript" language="JavaScript">
	...
	var fheight="405";
	var loadurl="index.php?case=file&act=piclist&admin_dir=<?php echo config::getadmin('admin_dir');?>";
	var loadlangurl="index.php?case=file&act=piclanglist&admin_dir=<?php echo config::getadmin('admin_dir');?>";
	...
	function picload(amid,page){
		...
		var loadingurl=loadurl + '&amid=' + amid + '&page=' + page + '&freshid=' + Math.random()+ '&lang=' + $('#amid').val();
	...
	function piclangload(lang,page){
		if (lang=="0") {
			$('#albumlist').html(filemanage_filecheck_select_allno);
			return false;
		}
		loadlangurl=loadlangurl + '&lang=' + lang + '&page=' + page;
		$.get(loadlangurl, function(data){
			$('#albumlist').html(data);
		});
	}
...
<div id="select" class="windowselecttop">
	<span id="amidlist">
	{lang_admin('gallery_list')}：<select size="1" name="amid" id="amid"  onchange="piclangload(this.value,1)" class="form-control">
		<option value="0">{lang_admin('please_select_the_gallery')}</option>
                     <?php foreach(lang::getlang() as $k => $v) { ?>
  <option value="<?php echo $v['langurlname'];?>"><?php echo $v['langurlname'];?></option>
  <?php } ?>
			</select>
