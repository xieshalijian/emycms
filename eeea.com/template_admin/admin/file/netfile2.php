...
<title>{lang_admin('network_video')}</title>
	...
	$(window).load(function(){
		...
		$('#submitbotton').click(function() {
            ...
            if(!/^http(s*):\/\//i.test($('#upfilepath').val())){
                alert("{lang_admin('must_start_with_http')}");
                $('#upfilepath').focus();
                return false;
            }
...
<body class="bodyflow">
<form name="upfile" id="upfile" method="post" action="index.php?case=file&act=netfilesave2&admin_dir={get('admin_dir',true)}" >
		...
		<div class="maneditcontent">
			<table class="formtablewin">
				<tr class="trstyle2">
					...
					<td class="trtitle02"><input id="upfilepath" type="text" name="upfilepath" maxlength="200" size="30" class="infoInput  form-control">
				    {lang_admin('must_start_with_http')}</td>
				</tr>
                 <tr class="trstyle2">
				  <td class="trtitle01"></td>
					<td class="trtitle02" id="upfilepath">
                        {lang_admin('automatic_playback')}：<input type="checkbox" name="autoplay" id="autoplay" value="1" />
				    {lang_admin('width')}
