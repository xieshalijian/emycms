...
<div class="modal fade" id="template-category-tag" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <ul class="nav nav-tabs" role="tablist" name="navigation_modal_tabs_category">
            ...
            <div class="modal-body">
                ...
                <!-- 设置 -->
                <div class="tab-content" name="category_modal_show">
            ...
            <div class="modal-footer">
                ...
                <button name="savecategory" type="button" class="btn btn-primary" data-dismiss="modal"><?php echo lang_admin('preservation');?>
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="template-old-category-tag" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button"  name="close" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <ul class="nav nav-tabs" role="tablist">
                    <li role="presentation" class="active">
                        <a href="#tag-config" aria-controls="tag-config" role="tab" data-toggle="tab">
                            <?php echo lang_admin('set_up');?>
                        </a>
                    </li>
                    <li role="presentation" name="tag-template" >
                        <a href="#tag-template" aria-controls="tag-template" role="tab" data-toggle="tab">
            ...
            <div class="modal-body">
                ...
                <div class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane active" id="tag-config">
                        <form method="post" name="frmcategory" id="frmcategory" action="">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-6 col-sm-7 col-md-7 col-lg-8 text-left">
                                    <?php echo form::select('catid', isset($tag_config['catid'])?$tag_config['catid']:0, category::option());?>
                                </div>
                    ...
                    <!-- 标签 -->
                    <div role="tabpanel" class="tab-pane" id="tag-template">
...
<script type="text/javascript">
    var template_getcategory='<?php echo url("template/getcategory");?>';
    var currenteditor1;
    $(document).ready(function () {
        var isoldcategory=false;
        $('body.edit .visual-right').on("click","[data-target='#template-category-tag']",function(e) {
            ...
            if(_tag_buymodules.test(eText) ||  _tag_modules.test(eText) ) {
                ...
                $('#template-category-tag').attr('style','display:block');
                $('#template-category-tag').addClass("modal-right");
                //打开标签选中
                //cmseasy_buymodules_category_common_v-common-0001_1
                $.ajaxSetup (  { async: true });
                $.post('<?php echo url("template/getmodulestag");?>', {'tag': eText, 'num': 1}, function (res) {
                    //隐藏old版栏目弹出框
                    $('#template-old-category-tag').modal('hide');
                    //弹出框的导航栏增加
           ...
           /*     $('#template-category-tag').attr('style','display:none');*/
                ...
                $('#template-old-category-tag').modal('show');
            }
        });

        //针对模块拖出组件  创建新的插件
        $('body.edit .visual-right').on("click","[data-target='#template-old-category-tag']",function(e) {
            e.preventDefault();
            currenteditor1 = $(this).parent().parent().parent().siblings('.view');
            var eText = currenteditor1.find('.tagname').html();
            //打开的老的弹出框
        ...
        $("[name=tagimg]").click(function(e) {
            $("#frmcategory .tagtemplate").val($(this).data("tagname"));
        ...
        $("[name=savecategory]").click(function(e) {
            e.preventDefault();
            if (isoldcategory){
                ...
                if( $("#frmcategory .tagtemplate").val() == ''){
                ...
                $("input[name='tag-tabid-category']").each(function(){
                    var id=$(this).val();
                    if($('#frmcategory'+id+' [name=textnum]').val() == ''){
                        alert("<?php echo lang_admin('please_fill_in_the_text_restrictions');?>");
                        istab($(this).val());
                        $('#frmcategory'+id+' [name=textnum]').focus();
                        return false;
                    }
                    if($('#frmcategory'+id+' [name=titlenum]').val() == ''){
                        alert("<?php echo lang_admin('please_fill_in_the_caption_text_restriction');?>");
                        istab(id);
                        $('#frmcategory'+id+' [name=titlenum]').focus();
                        return false;
                    }
                    if($('#frmcategory'+id+' [name=catid]').val() == '0'){
                        alert("<?php echo lang_admin('please_select_the_column');?>");
                        istab(id);
                        $('#frmcategory'+id+' [name=catid]').focus();
                        return false;
                    }
                    data = $('#frmcategory'+id).serialize();
                    ...
                    $.post(savemoduletagurl,data,function(res){
                        $('#frmcategory'+id)[0].reset();
                        $('#frmcategory'+id+' .id').val('');
                        savemodule('frmcategory',id,res,currenteditor1);
                        ready_all();
                    });
