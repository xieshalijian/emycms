...
<div class="modal fade" id="template-special-shop-tag" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <ul class="nav nav-tabs" role="tablist" name="navigation_modal_tabs_shopspecial">
            ...
            <div class="modal-body">
                ...
                <!-- 设置 -->
                <div class="tab-content" name="shop_special_modal_show">
            ...
            <div class="modal-footer">
                ...
                <button name="saveshopSpecial" type="button" class="btn btn-primary" data-dismiss="modal"><?php echo lang_admin('preservation');?>
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="template-old-special-shop-tag" tabindex="-1" role="dialog"
     aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane active" id="tag-special-config">
                      <form method="post" name="frmshopspecial" id="frmshopspecial" action="">
...
<script type="text/javascript">
    var shopspecialHandle;
    var template_getshopspecial='<?php echo url("template/getspecial/shop/1");?>';
    $(document).ready(function () {
        var isoldshopspecial=false;
        $('body.edit .visual-right').on("click","[data-target='#template-special-shop-tag']",function(e) {
            e.preventDefault();
            shopspecialHandle = $(this).parent().parent().parent().parent().find('.view');
            var eText = shopspecialHandle.find('.tag .tagname').html();
            ...
            if(_tag_buymodules.test(eText) ||  _tag_modules.test(eText) ) {
                ...
                $('#template-special-shop-tag').attr('style','display:block');
                $('#template-special-shop-tag').addClass("modal-right");
                $.ajaxSetup (  { async: true });
                $.post('<?php echo url("template/getmodulestag");?>', {'tag': eText, 'num': 1}, function (res) {
                    //隐藏old版栏目弹出框
                    $('#template-old-special-shop-tag').modal('hide');
                    //弹出框的导航栏增加
                ...
                //打开的老的弹出框
                isoldshopspecial=true;
                //打开标签选中
                $("[name='tag-special-shop-template']").removeAttr("style");
                //隐藏新版栏目弹出框
                $.ajaxSetup (  { async: true });
                $.post('<?php echo url("template/gettag");?>',{'tag':eText},function(res){
                    //console.log(res);
                    //弹出框
                    $('#template-special-shop-tag').modal('hide');
                    if(res.tagname!="" && res.tagname!=undefined) {
                        shopspecialHandle.find('.tag .tagname').html(res.tagname);     //修改html的内容
                    }
                    $('#frmshopspecial .id').val(res.id);
                    $('#frmshopspecial .name').val(res.name);
                    $('#frmshopspecial .spid').val(res.setting.spid);
                    $('#frmshopspecial .spname').val(res.setting.spname);
                    $('#frmshopspecial .subtitle').val(res.setting.subtitle);
                    $('#frmshopspecial .spimage').val(res.setting.spimage);
                    $('#frmshopspecial .spcontent').val(res.setting.spcontent);
                    $('#frmshopspecial .len').val(res.setting.len);
                    $('#frmshopspecial .remarksname').val(res.setting.remarksname);
                    //循环选中
                ...
                $('#template-old-special-shop-tag').modal('show');
            }
        });

        $("[name=saveshopSpecial]").click(function(e) {
            e.preventDefault();
            if (isoldshopspecial) {
                data = $('#frmshopspecial').serialize();
                $.post('<?php echo url("template/savetag");?>', data, function (res) {
                    $('#frmshopspecial')[0].reset();
                    $('#frmshopspecial .id').val('');
                    shopspecialHandle.html(res);
                });
            }
            else {
                $("input[name='tag-tabid-shopspecial']").each(function(){
                    var id=$(this).val();
                    data = $('#frmshopspecial'+id).serialize();
                    ...
                    $.post(savemoduletagurl,data,function(res){
                        $('#frmshopspecial'+id)[0].reset();
                        $('#frmshopspecial'+id+' .id').val('');
                        savemodule('frmshopspecial',id,res,shopspecialHandle);
                        ready_all();
                    });
        ...
        $("[name=tagimg]").click(function(e) {
            $("#frmshopspecial .tagtemplate").val($(this).data("tagname"));
