...
var client = new hprose.HttpClient("index.php?case=hprose&act=license", [
        'chkMoney','BuyLicense'
    ]
);
...
function apply(domain, year,aid) {
    ...
    setProgInfo('<div class="progress">\n' +
        ...
        '\t\t\t\t\t\t<span class="alertinfo">正在购买授权</span>\n' +
        '\t\t\t\t\t</div>\n' +
        '\t\t\t\t</div>');

    client.BuyLicense(domain,year,aid,shopingtype).then(function (res) {
        console.info(res);
        if (res['code'] != 0) {
            setInfo(res['msg']);
            return Promise.reject(res);
        } else {
            setSuccessInfo('<center><h4 class="modal-title" id="buyModalLabel">授权购买成功！</h4><br><a href="index.php?case=license&act=getmycd&id='+res.data.id+'" target="_blank" class="shoppingcart">点击下载授权文件</a></center>');
        }
    }).catch(function (err) {
